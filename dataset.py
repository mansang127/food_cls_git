...
class LT_Dataset(Dataset):
    ...
    def __init__(self, root, txt, transform=None):
        ...
        with open(root+txt) as f:
            for line in f:
                self.img_path.append(os.path.join(root, line.split()[0])) #图片地址
                self.targets.append(int(line.split()[1])) #图片对应的类别
        ...
        sorted_classes = np.argsort(-np.array(cls_num_list_old)) #按种类数量从大到小排序，即[0]是数量最多的种类
        ...
        #class_data[j]包含j类标签在targets中的索引值
        #cls_num_list[j]包含j类标签的样本数量
...
class Food_LT(object):
    def __init__(self, distributed, root="", batch_size=60, num_works=40):
        ...
        transform_train = transforms.Compose([
            ...
            transforms.ColorJitter(brightness=0.375, contrast=0.2, saturation=0.3, hue=0),
